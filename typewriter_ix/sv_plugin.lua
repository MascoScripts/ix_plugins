...
net.Receive("MascoType::CreateDocumentInv", function(len, ply)
    ...
    if ply.AntiSpamTypeWriter > CurTime() then ply:Notify("You must wait 1 minute in between uses of the Typewriter. You must wait " .. math.ceil(timeRemaining) .. " more second(s).") return end

    ply.AntiSpamTypeWriter = CurTime() + MascoTypeWriter.AntiSpamTime
